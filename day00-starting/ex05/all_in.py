...
def get_state(capital_city):
    ...
    capital_city_abbreviation = lower_dictionnary_keys(
        reversed_capital_cities_dict
    ).get(capital_city.lower())
...
def lower_dictionnary_keys(dictionnary):
    return {key.lower(): value for key, value in dictionnary.items()}
